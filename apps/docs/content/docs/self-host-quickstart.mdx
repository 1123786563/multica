...
- **Backend**: [http://localhost:8280](http://localhost:8280)
...
```bash
multica setup self-host --server-url http://<your-server-address>:8280 --app-url http://<your-server-address>:3300
```
...
That defaults to `http://localhost:8280` (backend) and `http://localhost:3300` (frontend).
