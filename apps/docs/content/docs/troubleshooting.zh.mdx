...
1. **`MULTICA_SERVER_URL` 指错地址** —— 默认是 `ws://localhost:8280/ws`，self-host 要改成你自己的 server 地址
...
curl -i http://<server-host>:8280/health   # 直接戳 server
curl -i http://<server-host>:8280/readyz  # 连同 DB + migration readiness 一起检查
...
1. **Origin 校验失败** —— 你的前端域名不在 server 的 CORS 白名单里。默认白名单只包含 `localhost:3300/5173/5174`，self-host 到公网必须配 `FRONTEND_ORIGIN`
...
- `curl -i http://<server-host>:8280/ws` 应该返回 `101 Switching Protocols`（需要带 `Upgrade` header）
...
1. **Server 端口被占用**（默认 `8280`）
2. **Daemon health 端口被占用**（默认 `19514`，每个 profile 偏移一个 hash 值）
3. **Web dev server 端口冲突**（`3300` / `5173`）
